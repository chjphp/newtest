...
class Express
{
    ...
    */
     ...
     * 获取对应名称和对应传值的方法
    */
    private function expressname($type, $order)
    {
        //$name   = json_decode($this->getcontent("http://www.kuaidi100.com/autonumber/auto?num={$order}"), true);
		$name = json_decode($this->getcontent("http://www.kuaidi100.com/query?type={$type}&postid={$order}"), true);
		var_dump($name);die;
    ...
    */
    ...
    {
        $keywords = $this->expressname($type, $order);
        if (!$keywords) {
            return false;
        } else {
